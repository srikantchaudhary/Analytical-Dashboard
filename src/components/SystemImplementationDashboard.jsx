  ...
  const sysChartOptions = {
    ...
    scales: {
      x: { 
        stacked: true, 
        max: TOTAL_STATES, 
        ticks: { stepSize: 4, font: { size: 12 } },
        title: { display: true, text: `Number of states (total = ${TOTAL_STATES})`, font: { size: 12 } }
      },
      y: { stacked: true, grid: { display: false }, ticks: { font: { size: 13 } } }
    }
  };
    ...
    <div className="min-h-screen bg-[#f5f4f0] py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-gray-800">GA Wing Survey 2026-27</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
            <p className="text-xl font-semibold mb-1">System Implementation Comparison</p>
            <p className="text-sm text-gray-600 mb-6">
              Number of states with Yes / Partial / No status across all 4 IT systems
            </p>

            <div className="flex flex-wrap gap-6 text-sm text-gray-600 mb-6">
              <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{background: '#639922'}}></div>Implemented / Yes</div>
              <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{background: '#EF9F27'}}></div>Partial / Trial / Mixed</div>
              <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{background: '#E24B4A'}}></div>Not implemented / No access</div>
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-center py-4 font-medium text-gray-600">Implemented / Yes</th>
                  <th className="text-center py-4 font-medium text-gray-600">Partial / Mixed</th>
                  <th className="text-center py-4 font-medium text-gray-600">No / Physical</th>
                ...
                {systemsData.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-100 hover:bg-gray-50 last:border-none">
                      <td className="py-5 font-semibold">{row.name}</td>
                      <td className="text-center">
                        <span className="inline-block bg-[#EAF3DE] text-[#3B6D11] px-5 py-1.5 rounded-full text-xs font-medium">
                          {row.yes} states
                        </span>
                      </td>
                      <td className="text-center">
                        {row.partial > 0 ? (
                          <span className="inline-block bg-[#FAEEDA] text-[#854F0B] px-5 py-1.5 rounded-full text-xs font-medium">
                            {row.partial} states
                          </span>
                        ) : <span className="text-gray-400">0</span>}
                      </td>
                      <td className="text-center">
                        <span className="inline-block bg-[#FCEBEB] text-[#A32D2D] px-5 py-1.5 rounded-full text-xs font-medium">
                          {row.no} states
                        </span>
                      </td>
          ...
          <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
            <p className="text-xl font-semibold mb-1">States / Users Pending Submission</p>
            <p className="text-sm text-gray-600 mb-6">Respondents who have not filled the form</p>

            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-[#f5f4f0] p-6 rounded-2xl text-center">
                <div className="text-4xl font-semibold">{TOTAL_STATES}</div>
                <div className="text-sm text-gray-500 mt-1">Total States</div>
              </div>
              <div className="bg-[#f5f4f0] p-6 rounded-2xl text-center">
                <div className="text-4xl font-semibold text-[#3B6D11]">18</div>
                <div className="text-sm text-gray-500 mt-1">Submitted</div>
              </div>
              <div className="bg-[#f5f4f0] p-6 rounded-2xl text-center">
                <div className="text-4xl font-semibold text-[#A32D2D]">10</div>
                <div className="text-sm text-gray-500 mt-1">Pending</div>
              </div>
              <div className="bg-[#f5f4f0] p-6 rounded-2xl text-center">
                <div className="text-4xl font-semibold text-[#854F0B]">64%</div>
            ...
            <div className="h-16 mb-8">
              <Bar 
                ...
                options={{
                  ...
                  plugins: { legend: { display: false } },
                  scales: { 
                    x: { stacked: true, max: TOTAL_STATES, display: false }, 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pendingStates.map((state, index) => (
                <div key={index} className="flex gap-4 p-4 border border-gray-100 rounded-2xl hover:shadow-sm transition-shadow">
                  <div className="w-11 h-11 bg-red-100 text-red-700 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0">
                    {getInitials(state.name)}
                  </div>
                  <div>
                    <div className="font-medium leading-tight">{state.name}</div>
                    <div className="text-xs text-gray-500 mt-1">{state.reason}</div>
                  </div>
                </div>
              ))}
            </div>
